CREATE TABLE GFDR_BM_RPT_CFG_DTL(
    ...
    COLUMN_NAME VARCHAR(80)    COMMENT '字段编码' ,
    COLUMN_DESC VARCHAR(200)    COMMENT '字段中文名称' ,
    COLUMN_TYPE VARCHAR(200)    COMMENT '字段类型' ,
    SEQ_NO INT    COMMENT '字段顺序' ,
    HANDLE_METHOD VARCHAR(800)    COMMENT '处理方法' ,
    DEFAULT_VALUE VARCHAR(200)    COMMENT '为空默认值，字符串填写''123'',数字填写 1.2' ,
    COLUMN_STATUS VARCHAR(2)    COMMENT '是否有效0-有效 1-无效' ,
    IS_CACHE VARCHAR(2)    COMMENT '校验时是否需要缓存，1-需要缓存 2-不需要缓存' ,
    ...
    PRIMARY KEY (DATA_ID)
) COMMENT = '上报报表字段配置' ENGINE=InnoDB;
