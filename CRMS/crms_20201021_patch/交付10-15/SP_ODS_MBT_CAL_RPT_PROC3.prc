...
BEGIN
    ...
    FOR NATURAL_KEY_ROW IN NATURAL_KEY_SCUR
        LOOP
            ...
            COMPARE_STR_S2 := COMPARE_STR_S2 || ' AND A.' || NATURAL_KEY_STR || ' = B.' || NATURAL_KEY_STR;
            COMPARE_STR_S3 := COMPARE_STR_S3 || ' AND T1.' || NATURAL_KEY_STR || ' = T2.' || NATURAL_KEY_STR;
            COMPARE_STR_S4 := COMPARE_STR_S4 || ', T1.' || NATURAL_KEY_STR;
        END LOOP;
        
        COMPARE_STR_S2 := SUBSTR(COMPARE_STR_S2, 5);
        COMPARE_STR_S3 := SUBSTR(COMPARE_STR_S3, 5);
        COMPARE_STR_S4 := SUBSTR(COMPARE_STR_S4, 2);
