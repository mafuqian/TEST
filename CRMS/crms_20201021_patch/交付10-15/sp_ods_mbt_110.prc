...
BEGIN
    ...
    BEGIN
        ...
        IF TABLE_KEY IS NOT NULL THEN
                ...
                USING (SELECT A.*,
                              ...
                              CASE
                                  WHEN TRIM(A.G_EMP_STATUS || A.G_CPN_NAME || A.G_CPN_TYPE || A.G_INDUSTRY ||
                                            A.G_CPN_ADDR ||
                                            A.G_CPN_PC || A.G_CPN_DIST || A.G_CPN_TEL || A.G_OCCUPATION || A.G_TITLE ||
                                            A.G_TECH_TITLE || A.G_WORK_START_DATE) IS NOT NULL
                                      THEN
                                      CASE
                                          WHEN B.DATA_ID IS NOT NULL AND
                                               nvl(B.G_EMP_STATUS || B.G_CPN_NAME || B.G_CPN_TYPE || B.G_INDUSTRY ||
                                                   B.G_CPN_ADDR ||
                                                   B.G_CPN_PC || B.G_CPN_DIST || B.G_CPN_TEL || B.G_OCCUPATION ||
                                                   B.G_TITLE ||
                                                   B.G_TECH_TITLE ||
                                                   B.G_WORK_START_DATE, 0) <>
                              ...
                              --H段
                              CASE
                                  WHEN TRIM(A.H_RESI_STATUS || A.H_RESI_ADDR ||
                                            A.H_RESI_PC || A.H_RESI_DIST || A.H_HOME_TEL) IS NOT NULL
                                      THEN
                                      CASE
                                          WHEN B.DATA_ID IS NOT NULL AND
                                  ...
                                  WHEN nvl(B.B_INF_REC_TYPE || B.B_NAME || B.B_ID_TYPE || B.B_ID_NUM ||
                                      ...
                                      =
                                       nvl(A.B_INF_REC_TYPE || A.B_NAME || A.B_ID_TYPE || A.B_ID_NUM ||
                                           ...
                                           A.J_TAX_INCOME || A.J_INC_INFO_UP_DATE, 0)
                                      THEN
                                      '99'
                                  ELSE
                                      '20'
                       ...
                       FROM MBT_110 A
                           ...
                           WHERE RN = 1
                              ...
                              A.B_INF_SURC_CODE = B.B_INF_SURC_CODE AND A.DATA_ID = TABLE_KEY
                ) T2
                ...
                WHEN MATCHED THEN
                    ...
                    SET T1.IS_RPT          = GET_ISRPT_CHANGE(T1.IS_RPT,
                                                              (T2.B_UPDATE_FLAG || T2.D_UPDATE_FLAG ||
                                                               ...
                                                               T2.I_UPDATE_FLAG || T2.J_UPDATE_FLAG || '1')),
            ...
            DSC := '##计算子表报告时点' || TABLE_KEY || '##';
            DBMS_OUTPUT.PUT_LINE(DSC);
            BEGIN

                IF RPT_DATE_CODE_OUT = 99 OR RPT_DATE_CODE_OUT IS NULL THEN
                    DSC := '##主表的段没有变化，再看子表段有无变化确定时点' || TABLE_KEY || '##';
                    DBMS_OUTPUT.PUT_LINE(DSC);
                    ...
                    FROM (
                             SELECT DISTINCT OTH_ID_TYPE, OTH_ID_NUM, ALIAS
                             FROM (
                             ...
                             LEFT JOIN MBT_110_RPT T4
                                       ON T3.B_ID_TYPE = T4.B_ID_TYPE
                                           ...
                                           AND T3.B_INF_SURC_CODE = T4.B_INF_SURC_CODE
            ...
            */
            ...
            BEGIN
                ...
                ELSE
                    ...
                    BEGIN
                        ...
                        USING (
                            SELECT A.*,
                                   ...
                                   --E段
                                   CASE
                                       WHEN D.DATA_ID IS NULL AND TRIM(A.E_MARI_STATUS || A.E_SPO_NAME ||
                                                                       A.E_SPO_ID_TYPE || A.E_SPO_ID_NUM ||
                                                                       A.E_SPO_TEL || A.E_SPS_CMPY_NM ||
                                                                       A.F_EDU_LEVEL) IS NULL
                                           THEN
                                           '0'
                                       ELSE
                                           '1'
                                   ...
                                   --G段
                                   CASE
                                       WHEN D.DATA_ID IS NULL AND TRIM(A.G_EMP_STATUS || A.G_CPN_NAME ||
                                   ...
                                   --H段
                                   CASE
                                       WHEN D.DATA_ID IS NULL AND TRIM(A.H_RESI_STATUS || A.H_RESI_ADDR ||
                                                                       A.H_RESI_PC || A.H_RESI_DIST ||
                                               ...
                                               WHEN nvl(B.E_MARI_STATUS || B.E_SPO_NAME || B.E_SPO_ID_TYPE ||
                                                   ...
                                                   CASE
                                                       WHEN nvl(C.E_MARI_STATUS || C.E_SPO_NAME || C.E_SPO_ID_TYPE ||
                                                                C.E_SPO_ID_NUM ||
                                                                C.E_SPO_TEL || C.E_SPS_CMPY_NM || C.F_EDU_LEVEL, 0)
                                                           <>
                                                            nvl(A.E_MARI_STATUS || A.E_SPO_NAME || A.E_SPO_ID_TYPE ||
                                                                A.E_SPO_ID_NUM ||
                                                                A.E_SPO_TEL || A.E_SPS_CMPY_NM || A.F_EDU_LEVEL, 0)
                                   ...
                                   /*CASE
                                       WHEN D.DATA_ID IS NOT NULL AND
                                           ...
                                           CASE
                                               WHEN nvl(B.G_EMP_STATUS || B.G_CPN_NAME ||
                                                        B.G_CPN_TYPE || B.G_INDUSTRY || B.G_CPN_ADDR ||
                                                        B.G_CPN_PC || B.G_CPN_DIST || B.G_CPN_TEL ||
                                                        B.G_OCCUPATION || B.G_TITLE ||
                                                        B.G_TECH_TITLE || B.G_WORK_START_DATE, 0)
                                                   <>
                                                    nvl(A.G_EMP_STATUS || A.G_CPN_NAME ||
                                                        A.G_CPN_TYPE || A.G_INDUSTRY || A.G_CPN_ADDR ||
                                                        A.G_CPN_PC || A.G_CPN_DIST || A.G_CPN_TEL ||
                                                        A.G_OCCUPATION || A.G_TITLE || A.G_TECH_TITLE ||
                                                        A.G_WORK_START_DATE, 0)
                                                   THEN
                                                   CASE
                                                       WHEN nvl(C.G_EMP_STATUS || C.G_CPN_NAME ||
                                                                C.G_CPN_TYPE || C.G_INDUSTRY || C.G_CPN_ADDR ||
                                                                C.G_CPN_PC || C.G_CPN_DIST || C.G_CPN_TEL ||
                                                                C.G_OCCUPATION || C.G_TITLE ||
                                                                C.G_TECH_TITLE || C.G_WORK_START_DATE, 0)
                                                           <>
                                                            nvl(A.G_EMP_STATUS || A.G_CPN_NAME ||
                                                                A.G_CPN_TYPE || A.G_INDUSTRY || A.G_CPN_ADDR ||
                                                                A.G_CPN_PC || A.G_CPN_DIST || A.G_CPN_TEL ||
                    ...
                    BEGIN
                        ...
                        USING (SELECT A.DATA_ID,
                                      CASE
                                          WHEN B.DATA_ID IS NULL THEN
                                              '10'
                                          WHEN (B.B_NAME || B.B_INF_SURC_CODE ||
                    ...
                    --更新时点，子表（段）的变化只能决定更新时点，同时更新该段的变化标识，没有改动时，该段无法决定是新增还是不报送的时点）
                    DSC := '##根据子段更新报告时点##';
                    DBMS_OUTPUT.PUT_LINE(DSC);
                    BEGIN
                        ...
                        WHEN MATCHED THEN
                            UPDATE
                            SET A.B_RPT_DATE_CODE = B.NEW_B_RPT_DATE_CODE,
                                --A.IS_RPT          = SUBSTR(A.IS_RPT, 1, INSTR(A.IS_RPT, '|')) || '1',
                                A.CHECK_FLAG      = 'N';
                    END;
