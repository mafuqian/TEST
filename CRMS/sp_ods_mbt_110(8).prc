        ...
        IF TABLE_KEY IS NOT NULL THEN
            ...
            BEGIN
                ...
                USING (SELECT A.*,
                              --B段
                              CASE
                                  WHEN TRIM(A.B_NAME || A.B_CUSTOMER_TYPE || A.B_CIMOC) IS NOT NULL THEN
                                      CASE
                                          WHEN --B.DATA_ID IS NOT NULL AND
                                                  NVL(B.B_NAME || B.B_CUSTOMER_TYPE || B.B_CIMOC, 0) <>
                                                  NVL(A.B_NAME || A.B_CUSTOMER_TYPE || A.B_CIMOC, 0)
                                              THEN
                                              '1'
                                          ELSE
                                              '0'
                              ...
                              CASE
                                  WHEN TRIM(A.D_SEX || A.D_DOB || A.D_NATION || A.D_HOUSE_ADD || A.D_HH_DIST ||
                                      ...
                                      CASE
                                          WHEN --B.DATA_ID IS NOT NULL AND
                                                  NVL(B.D_SEX || B.D_DOB || B.D_NATION || B.D_HOUSE_ADD ||
                                                      B.D_HH_DIST ||
                              ...
                              CASE
                                  WHEN TRIM(A.E_MARI_STATUS || A.E_SPO_NAME || A.E_SPO_ID_TYPE || A.E_SPO_ID_NUM ||
                                            A.E_SPO_TEL || A.E_SPS_CMPY_NM) IS NOT NULL
                                      THEN
                                      CASE
                                          WHEN --B.DATA_ID IS NOT NULL AND
                                                  NVL(B.E_MARI_STATUS || B.E_SPO_NAME || B.E_SPO_ID_TYPE ||
                                                      B.E_SPO_ID_NUM || B.E_SPO_TEL || B.E_SPS_CMPY_NM, 0)
                                                  <>
                                                  NVL(A.E_MARI_STATUS || A.E_SPO_NAME || A.E_SPO_ID_TYPE ||
                                                      A.E_SPO_ID_NUM || A.E_SPO_TEL || A.E_SPS_CMPY_NM, 0)
                                              THEN
                                              '1'
                                          ELSE
                                              '0'
                                      ...
                                      CASE
                                          WHEN --B.DATA_ID IS NOT NULL AND
                                                  NVL(B.G_EMP_STATUS || B.G_CPN_NAME || B.G_CPN_TYPE || B.G_INDUSTRY ||
                                                      ...
                                                      B.G_OCCUPATION || B.G_TITLE || B.G_TECH_TITLE ||
                                                      B.G_WORK_START_DATE,
                                                      0) <>
                              ...
                              CASE
                                  WHEN TRIM(A.H_RESI_STATUS || A.H_RESI_ADDR || A.H_RESI_PC || A.H_RESI_DIST ||
                                      ...
                                      CASE
                                          WHEN --B.DATA_ID IS NOT NULL AND
                                                  NVL(B.H_RESI_STATUS || B.H_RESI_ADDR || B.H_RESI_PC ||
                                                      B.H_RESI_DIST ||
                                                      B.H_HOME_TEL, 0) <>
                                                  NVL(A.H_RESI_STATUS || A.H_RESI_ADDR || A.H_RESI_PC ||
                                                      A.H_RESI_DIST ||
                                                      A.H_HOME_TEL, 0)
                                              THEN
                                              '1'
                              ...
                              CASE
                                  WHEN B.DATA_ID IS NULL THEN
                                      '10'
                                  --不参与变化量计算的字段 ：C_ID_INFO_UP_DATE、D_FCS_INFO_UP_DATE、E_SPS_INFO_UP_DATE、F_EDU_INFO_UP_DATE、G_OCTPN_INFO_UP_DATE、H_RESI_INFO_UP_DATE、I_MLG_INFO_UP_DATE、J_INC_INFO_UP_DATE、J_TAX_INCOME、J_ANNL_INC
                                  WHEN (NVL(B.B_NAME || B.B_CIMOC || B.B_CUSTOMER_TYPE || B.D_SEX ||
                                            ...
                                            B.I_MAIL_DIST, 0)
                                      <>
                                        NVL(A.B_NAME || A.B_CIMOC || A.B_CUSTOMER_TYPE || A.D_SEX ||
                                            A.D_DOB || A.D_NATION || A.D_HOUSE_ADD || A.D_HH_DIST ||
                                            A.D_CELL_PHONE || A.D_EMAIL ||
                       ...
                       FROM MBT_110 A
                                ...
                                LEFT JOIN (SELECT '1' AS FLAG, B_ID_TYPE, B_ID_NUM, B_INF_SURC_CODE
                                           FROM (SELECT CASE
                                                            WHEN NVL(A.ALIAS || A.OTH_ID_TYPE || A.OTH_ID_NUM, 0) <>
                                                                 NVL(B.ALIAS || B.OTH_ID_TYPE || B.OTH_ID_NUM, 0)
                                                                THEN '1'
                                                            ELSE '0' END AS                       FLAG,
                                                        NVL(A.B_ID_TYPE, B.B_ID_TYPE)             B_ID_TYPE,
                                                        NVL(A.B_ID_NUM, B.B_ID_NUM)               B_ID_NUM,
                                                        NVL(A.B_INF_SURC_CODE, B.B_INF_SURC_CODE) B_INF_SURC_CODE
                                                 FROM MBT_110_C A
                                                          FULL JOIN (SELECT A.*
                                                                    ...
                                                                    ON A.ALIAS = B.ALIAS AND
                                                                       A.OTH_ID_TYPE = B.OTH_ID_TYPE AND
                                                                       A.OTH_ID_NUM = B.OTH_ID_NUM AND
                                                                       A.B_ID_TYPE = B.B_ID_TYPE AND
                                                                       A.B_ID_NUM = B.B_ID_NUM AND
                                                                       A.B_INF_SURC_CODE = B.B_INF_SURC_CODE)
                                           WHERE FLAG > 0
                                           GROUP BY B_ID_TYPE, B_ID_NUM, B_INF_SURC_CODE) CC
                ...
                ON (T2.B_ID_TYPE = T1.B_ID_TYPE AND T2.B_ID_NUM = T1.B_ID_NUM AND
                    T1.B_INF_SURC_CODE = T2.B_INF_SURC_CODE AND
                    T1.DATA_ID = TABLE_KEY)
                WHEN MATCHED THEN
                    UPDATE
                    ...
                    BEGIN
                        ...
                        USING (
                            SELECT A.*,
                                   ...
                                   CASE
                                       WHEN D.DATA_ID IS NULL AND
                                            TRIM(A.B_NAME || A.B_CUSTOMER_TYPE || A.B_CIMOC) IS NOT NULL THEN
                                           '1'
                                       ELSE
                                           '0'
                                   ...
                                   --F段
                                   CASE
                                       WHEN D.DATA_ID IS NULL AND TRIM(A.F_EDU_LEVEL || A.F_ACA_DEGREE) IS NOT NULL THEN
                                           '1'
                                       ELSE
                                           '0'
                                   ...
                                   CASE
                                       WHEN D.DATA_ID IS NULL AND
                                            TRIM(A.I_MAIL_ADDR || A.I_MAIL_PC || A.I_MAIL_DIST) IS NOT NULL
                                           THEN
                                           '1'
                                       ELSE
                                           '0'
                                   ...
                                   --J段
                                   CASE
                                       WHEN D.DATA_ID IS NULL AND TRIM(A.J_ANNL_INC || A.J_TAX_INCOME) IS NOT NULL
                                           THEN
                                           '1'
                                       ELSE
                                           '0'
                                ...
                                /*CASE
                                    WHEN D.DATA_ID IS NOT NULL AND TRIM(A.B_NAME || A.B_CUSTOMER_TYPE) IS NOT NULL
                                        THEN*/
                                   CASE
                                       WHEN NVL(B.B_NAME || B.B_CUSTOMER_TYPE || B.B_CIMOC, 0) <>
                                            NVL(A.B_NAME || A.B_CUSTOMER_TYPE || A.B_CIMOC, 0)
                                           THEN
                                           CASE
                                               WHEN NVL(C.B_NAME || C.B_CUSTOMER_TYPE || C.B_CIMOC, 0) <>
                                                    NVL(A.B_NAME || A.B_CUSTOMER_TYPE || A.B_CIMOC, 0)
                                                   THEN
                                                   '1'
                                               ELSE
                                ...
                                /*CASE
                                    WHEN D.DATA_ID IS NOT NULL AND
                                         TRIM(A.E_MARI_STATUS || A.E_SPO_NAME || A.E_SPO_ID_TYPE || A.E_SPO_ID_NUM ||
                                              A.E_SPO_TEL || A.E_SPS_CMPY_NM) IS NOT NULL THEN*/
                                   CASE
                                       WHEN NVL(B.E_MARI_STATUS || B.E_SPO_NAME || B.E_SPO_ID_TYPE ||
                                           ...
                                           CASE
                                               WHEN NVL(C.E_MARI_STATUS || C.E_SPO_NAME || C.E_SPO_ID_TYPE ||
                                                        C.E_SPO_ID_NUM || C.E_SPO_TEL || C.E_SPS_CMPY_NM, 0)
                                                   <>
                                                    NVL(A.E_MARI_STATUS || A.E_SPO_NAME || A.E_SPO_ID_TYPE ||
                                                        A.E_SPO_ID_NUM || A.E_SPO_TEL || A.E_SPS_CMPY_NM, 0)
                                                   THEN
                            ...
                            FROM ODS_MBT_110 A
                                     ...
                                     LEFT JOIN MBT_110 D --需要考虑mbt表中有用户新增的和ods中一样的数据
                        ...
                        WHEN MATCHED THEN
                            ...
                            SET T1.B_NAME               = CASE
                                ...
                                T1.E_SPS_INFO_UP_DATE   = CASE
                                                              WHEN T2.E_UPDATE_FLAG = '1' THEN V_T_DATE
                                ...
                                T1.F_EDU_INFO_UP_DATE   = CASE
                                                              WHEN T2.F_UPDATE_FLAG = '1' THEN V_T_DATE
                                ...
                                T1.G_OCTPN_INFO_UP_DATE = CASE
                                                              WHEN T2.G_UPDATE_FLAG = '1' THEN V_T_DATE
