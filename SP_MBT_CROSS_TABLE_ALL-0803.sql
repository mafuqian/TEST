...
prompt Created by 77164 on 2020骞?鏈?鏃?
set define off
spool SP_MBT_CROSS_TABLE_ALL鈥斺€?.log
...
BEGIN
  ...
  BEGIN
    ...
    ELSIF V_RPT_DATA_NO = '2019MBT210A006' THEN
      ...
      declare
        cursor table_cur is
          select * from mbt_210 a where 1=1 and a.data_status != 22 and a.B_RPT_DATE=V_RPT_DATE AND a.B_RPT_DATE_CODE NOT IN ('88','99') 
          and not exists
          (select 1 from (
...
BEGIN
  ...
  BEGIN
    ...
    IF V_RPT_DATA_NO = '2019MBT220A010' THEN
      ...
      declare
        cursor table_cur is
          select * from mbt_220 a where 1=1 and a.data_status != 22 and a.B_RPT_DATE=V_RPT_DATE AND a.B_RPT_DATE_CODE NOT IN ('88','99') 
          and not exists
          (select 1 from (
                           SELECT DISTINCT B_NAME,B_ID_TYPE,B_ID_NUM FROM (
                             select B_NAME,B_ID_TYPE,B_ID_NUM  from mbt_110 where B_RPT_DATE = V_RPT_DATE
...
BEGIN
  ...
  BEGIN
    ...
    IF V_RPT_DATA_NO = '2019MBT410A019' THEN
      ...
      declare
        cursor table_cur is
          select * from mbt_410 a where 1=1 and a.data_status != 22 and a.B_RPT_DATE=V_RPT_DATE AND a.B_RPT_DATE_CODE NOT IN ('88','99') 
          and not exists
          (select 1 from (
                           SELECT DISTINCT B_ENT_NAME,B_ENT_CERT,B_ENT_CERT_NUM FROM (
                             select B_ENT_NAME,B_ENT_CERT,B_ENT_CERT_NUM from mbt_310 where B_RPT_DATE = V_RPT_DATE
                             ...
                             union all
                             select B_ENT_NAME,B_ENT_CERT,B_ENT_CERT_NUM from  mbt_310_rpt )
                         ) b where A.B_NAME=B.B_ENT_NAME AND A.B_ID_TYPE=B.B_ENT_CERT AND A.B_ID_NUM=B.B_ENT_CERT_NUM);
      begin
        for REC in table_cur loop
...
BEGIN
  ...
  BEGIN
    ...
    IF V_RPT_DATA_NO = '2019MBT420A025' THEN
      ...
      declare
        cursor table_cur is
          select * from mbt_420 a where 1=1 and a.data_status != 22 and a.B_RPT_DATE=V_RPT_DATE AND a.B_RPT_DATE_CODE NOT IN ('88','99') 
          and not exists
          (select 1 from (
                           SELECT DISTINCT B_ENT_NAME,B_ENT_CERT,B_ENT_CERT_NUM FROM (
                             select B_ENT_NAME,B_ENT_CERT,B_ENT_CERT_NUM from mbt_310 where B_RPT_DATE = V_RPT_DATE
...
BEGIN
  ...
  BEGIN
    ...
    IF V_RPT_DATA_NO = '2019MBT440A028' THEN
      ...
      declare
        cursor table_cur is
          select * from mbt_440 a where 1=1 and a.data_status != 22 and a.B_RPT_DATE=V_RPT_DATE AND a.B_RPT_DATE_CODE NOT IN ('88','99') 
          and not exists
          (select 1 from (
                           SELECT DISTINCT B_ENT_NAME,B_ENT_CERT,B_ENT_CERT_NUM FROM (
                             select B_ENT_NAME,B_ENT_CERT,B_ENT_CERT_NUM from mbt_310 where B_RPT_DATE = V_RPT_DATE
...
BEGIN
  ...
  BEGIN
    ...
    IF V_RPT_DATA_NO = '2019MBT510A034' THEN
      ...
      declare
        cursor table_cur is
          select * from mbt_510 a where 1=1 and a.data_status != 22 and a.B_RPT_DATE=V_RPT_DATE and a.B_RPT_DATE_CODE NOT IN ('88','99') AND B_INFO_ID_TYPE = '2' 
          and not exists
          (select 1 from (
                           SELECT DISTINCT B_ENT_NAME,B_ENT_CERT,B_ENT_CERT_NUM FROM (
                             select B_ENT_NAME,B_ENT_CERT,B_ENT_CERT_NUM  from mbt_310 where B_RPT_DATE = V_RPT_DATE
...
BEGIN
  ...
  BEGIN
    ...
    IF V_RPT_DATA_NO = '2019MBT510A016' THEN
      ...
      declare
        cursor table_cur is
          select * from mbt_510 a where 1=1 and a.data_status != 22 and a.B_RPT_DATE=V_RPT_DATE and a.B_RPT_DATE_CODE NOT IN ('88','99') AND B_INFO_ID_TYPE = '1' 
          and not exists
          (select 1 from (
                           SELECT DISTINCT B_NAME,B_ID_TYPE,B_ID_NUM FROM (
                             select B_NAME,B_ID_TYPE,B_ID_NUM  from mbt_110 where B_RPT_DATE = V_RPT_DATE
                             ...
                             union all
                             select B_NAME,B_ID_TYPE,B_ID_NUM from  mbt_110_rpt )
                         ) b where A.B_NAME=B.B_NAME AND A.B_CERT_TYPE=B.B_ID_TYPE AND A.B_CERT_NUM=B.B_ID_NUM);
      begin
        for REC in table_cur loop
